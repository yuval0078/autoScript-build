...
class WaveformWidget(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        # Store initial click position for direction detection
        self.click_x = x
        ...
        # If markers are at the same position (or very close), don't decide yet
        # We'll determine which to drag based on movement direction in mouseMoveEvent
        if abs(start_x - end_x) < 2:
            self.dragging = 'overlap'
        elif abs(x - start_x) < 10:
            self.dragging = 'start'
        elif abs(x - end_x) < 10:
            self.dragging = 'end'
        else:
    ...
    def mouseMoveEvent(self, event):
        ...
        # Handle dragging
        if self.dragging == 'overlap':
            # Determine which marker to drag based on movement direction
            if x > self.click_x:
                # Dragging right -> drag end marker
                self.dragging = 'end'
            else:
                # Dragging left -> drag start marker
                self.dragging = 'start'
...
class NewExperimentWizard(QWidget):
    ...
    def init_ui(self):
        ...
        self.slice_list.setDragEnabled(True)
        self.slice_list.setAcceptDrops(True)
        self.slice_list.setDropIndicatorShown(True)
        self.slice_list.setDragDropMode(QListWidget.InternalMove)
        self.slice_list.model().rowsMoved.connect(self.on_slices_reordered)
        ...
        buttons_layout.addWidget(self.btn_edit_slices)
        
        self.btn_insert_word = QPushButton("➕ Insert Word")
        self.btn_insert_word.clicked.connect(self.insert_word)
        self.btn_insert_word.setEnabled(False)
        buttons_layout.addWidget(self.btn_insert_word)
        
        self.btn_delete_slice = QPushButton("🗑 Delete Selected")
        self.btn_delete_slice.clicked.connect(self.delete_word)
        self.btn_delete_slice.setEnabled(False)
        buttons_layout.addWidget(self.btn_delete_slice)
    ...
    def select_group(self, item):
        ...
        self.btn_insert_word.setEnabled(True)
    ...
    def delete_word(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            # Select next word or previous if last was deleted
            new_row = min(current_row, self.slice_list.count() - 1)
            if new_row >= 0:
                self.slice_list.setCurrentRow(new_row)

    def insert_word(self):
        """Insert a new word with a zero-length slice at the beginning of the audio"""
        if not self.current_group:
            return
        
        group_data = self.audio_groups[self.current_group]
        
        # Create a new segment with zero length at position 0
        new_segment = {
            'start': 0,
            'end': 0,
            'duration': 0,
        ...
        # Insert at the beginning of the segments list
        group_data['segments'].insert(0, new_segment)
        
        # Insert empty text entry at the beginning
        group_data['text_words'].insert(0, '')
        
        # Refresh the UI
        self.select_group(self.group_list.currentItem())
        
        # Select the newly inserted word
        self.slice_list.setCurrentRow(0)
        
        # Update text edit to show the new empty line at the top
    ...
    def on_slices_reordered(self, parent, start, end, destination, row):
        """Handle reordering of slices via drag-and-drop"""
        if not self.current_group:
            return
        ...
        # Qt's row parameter is where the item was dropped
        # start is the original position
        dest_index = row if row < start else row - 1
        
        # Move the segment
        segment = group_data['segments'].pop(start)
        group_data['segments'].insert(dest_index, segment)
        
        # Move the corresponding text word if it exists
        if start < len(group_data['text_words']):
            text_word = group_data['text_words'].pop(start)
            # Insert at destination, creating empty entries if needed
            while len(group_data['text_words']) < dest_index:
                group_data['text_words'].append('')
            if dest_index < len(group_data['text_words']):
                group_data['text_words'].insert(dest_index, text_word)
            else:
                group_data['text_words'].append(text_word)
        
        # Update the text editor to reflect new order
        self.text_edit.setPlainText('\n'.join(group_data['text_words']))
